...
# Set log file
log_file = open("logB1.txt", "w")

def log_print(*args):
    message = " ".join(map(str, args))
    log_file.write(message + "\n")  # Write to log file simultaneously
...
# Communication range and speed of the vehicle
comm_range = 7
car_speed = 1

# Randomly generate signatures and address information
...
# Poisson distribution random point scattering
...
def initialize_rsus(car_position):
    ...
    # The 6th RSU is in the direction of the vehicle but initially out of communication range
    distance = random.uniform(comm_range + 1, 1.5 * comm_range)
...
def admission_process(new_rsu, new_rsu_add):
    ...
    if valid_responses > len(current_rsus) // 3:
        log_print(f"RSU {new_rsu} verified successfully, admitted")
...
def vehicle_moving(is_legal):
    ...
    for step in range(10):  # Simulate 10 time steps
        ...
        # Detect if a new RSU has entered the communication range
        for i, rsu_position in enumerate(rsu_positions):
            if calculate_distance(car_position, rsu_position) < comm_range:
                if rsus[i] not in current_rsus:
                    ...
                    start_time = time.time()  # Start time of the admission process
                    if admission_process(rsus[i], adds[i]):
                        elapsed_time = time.time() - start_time  # Calculate the time of the admission process
                        ret = (1, elapsed_time)
                    else:
                        elapsed_time = time.time() - start_time  # Calculate the time of the admission process
                        ret = (0, elapsed_time)
...
def run_experiments(n, filename, is_legal):
    ...
    with open(filename, "w") as result_file:
        ...
        result_file.write("\n")
        result_file.write(f"Total number: {n}\n")
        result_file.write(f"Number of successes: {success_count}\n")
...
def record_experiments():
    ...
    for count in experiment_counts:
        ...
        run_experiments(count, f"result_invalid_rsu_time_without_limit_{count}.txt", is_legal=False)
